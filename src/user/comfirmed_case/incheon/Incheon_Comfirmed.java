...
public class Incheon_Comfirmed extends JFrame implements ActionListener {
	...
	public void useraddAll(DefaultTableModel model) {
		String sql = "select * from confirmed where PERSON LIKE '%인천%'";
		try {
	...
	public void getSearch(DefaultTableModel model, String combo, String word) throws Exception {
		String sql = "select * from confirmed where PERSON LIKE '%인천%' AND " + combo.trim() + " LIKE '" + word.trim()
				+ "'";
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (obj == b2) {
			new Incheon_guidelines("인천 방역지침", 930, 570);
		} else if (obj == b3) {
			useraddAll(model);
		} else if (obj == b1) {
			new Incheon_Search("인천 확진자 검색", 450, 250, this);
		}
	}
