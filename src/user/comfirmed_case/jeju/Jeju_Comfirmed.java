package user.comfirmed_case.jeju;
...
public class Jeju_Comfirmed extends JFrame implements ActionListener {
	...
	public Jeju_Comfirmed(String title, int width, int height) {
		this.setTitle(title);
	...
	public void useraddAll(DefaultTableModel model) {
		String sql = "select * from confirmed where PERSON LIKE '%제주%'";
		try {
	...
	public void getSearch(DefaultTableModel model, String combo, String word) throws Exception {
		String sql = "select * from confirmed where PERSON LIKE '%제주%' AND " + combo.trim() + " LIKE '" + word.trim()
				+ "'";
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (obj == b2) {
			new Jeju_guidelines("제주 방역지침", 710, 750);
		} else if (obj == b3) {
			useraddAll(model);
		} else if (obj == b1) {
			new Jeju_Search("제주 확진자 검색", 450, 250, this);
		}
	}
