...
public class Gyeongbuk_Comfirmed extends JFrame implements ActionListener {
	...
	public Gyeongbuk_Comfirmed(String title, int width, int height) {
		this.setTitle(title);
	...
	public void useraddAll(DefaultTableModel model) {
		String sql = "select * from confirmed where PERSON LIKE '%경북%'";
		try {
	...
	public void getSearch(DefaultTableModel model, String combo, String word) throws Exception {
		String sql = "select * from confirmed where PERSON LIKE '%경북%' AND " + combo.trim() + " LIKE '" + word.trim()
				+ "'";
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (obj == b2) {
			new Gyeongbuk_guidelines("경북 방역지침", 610, 670);
		} else if (obj == b3) {
			useraddAll(model);
		} else if (obj == b1) {
			new Gyeongbuk_Search("경북 확진자 검색", 450, 250, this);
		}
	}
