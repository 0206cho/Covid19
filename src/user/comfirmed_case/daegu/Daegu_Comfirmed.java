package user.comfirmed_case.daegu;
...
public class Daegu_Comfirmed extends JFrame implements ActionListener {
	...
	public Daegu_Comfirmed(String title, int width, int height) {
		this.setTitle(title);
	...
	public void useraddAll(DefaultTableModel model) {
		String sql = "select * from confirmed where PERSON LIKE '%대구%'";
		try {
	...
	public void getSearch(DefaultTableModel model, String combo, String word) throws Exception {
		String sql = "select * from confirmed where PERSON LIKE '%대구%' AND " + combo.trim() + " LIKE '" + word.trim()
				+ "'";
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (obj == b2) {
			new Daegu_guidelines("대구 방역지침", 725, 725);
		} else if (obj == b3) {
			useraddAll(model);
		} else if (obj == b1) {
			new Daegu_Search("대구 확진자 검색", 450, 250, this);
		}
	}
