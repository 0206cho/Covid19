package user.vaccinationstatus.ulsan;
...
public class Ulsan_Comfirmed extends JFrame implements ActionListener {
	...
	public Ulsan_Comfirmed(String title, int width, int height) {
		this.setTitle(title);
	...
	public void useraddAll(DefaultTableModel model) {
		String sql = "select * from vaccinationstatus where name LIKE '%울산광역시%'";
		try {
...
//				+ "'";
		...
		String sql = "SELECT * FROM vaccinationstatus WHERE name LIKE '%울산광역시%' AND name LIKE '%" + word + "%'";
	...
	public static void main(String[] args) {
		...
		new Ulsan_Comfirmed("울산 보건소 현황", 880, 615);
	}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (obj == b2) {
			new Ulsan_guidelines("울산 방역지침", 610, 670);
		} else if (obj == b3) {
			useraddAll(model);
		} else if (obj == b1) {
			new Ulsan_Search("울산 보건소 검색", 350, 250, this);
		}
	}
