package user.vaccinationstatus.gyeonggi;
...
public class Gyeonggi_Comfirmed extends JFrame implements ActionListener {
	...
	public Gyeonggi_Comfirmed(String title, int width, int height) {
		this.setTitle(title);
	...
	public void useraddAll(DefaultTableModel model) {
		String sql = "select * from vaccinationstatus where name LIKE '%경기도%'";
		try {
...
//				+ "'";
		...
		String sql = "SELECT * FROM vaccinationstatus WHERE name LIKE '%경기도%' AND name LIKE '%" + word + "%'";
	...
	public static void main(String[] args) {
		...
		new Gyeonggi_Comfirmed("경기 보건소 현황", 880, 615);
	}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (obj == b2) {
			new Gyeonggi_guidelines("경기 방역지침", 610, 670);
		} else if (obj == b3) {
			useraddAll(model);
		} else if (obj == b1) {
			new Gyeonggi_Search("경기 보건소 검색", 350, 250, this);
		}
	}
