package user.vaccinationstatus.gwangju;
...
public class Gwangju_Comfirmed extends JFrame implements ActionListener {
	...
	public Gwangju_Comfirmed(String title, int width, int height) {
		this.setTitle(title);
	...
	public void useraddAll(DefaultTableModel model) {
		String sql = "select * from vaccinationstatus where name LIKE '%광주광역시%'";
		try {
...
//				+ "'";
		...
		String sql = "SELECT * FROM vaccinationstatus WHERE name LIKE '%광주광역시%' AND name LIKE '%" + word + "%'";
	...
	public static void main(String[] args) {
		...
		new Gwangju_Comfirmed("광주 보건소 현황", 880, 615);
	}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (obj == b2) {
			new Gwangju_guidelines("광주 방역지침", 610, 670);
		} else if (obj == b3) {
			useraddAll(model);
		} else if (obj == b1) {
			new Gwangju_Search("광주 보건소 검색", 350, 250, this);
		}
	}
