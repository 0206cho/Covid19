...
// JFrame을 불러옴
	public IsolatedTogether() {
		...
		lblTogether = new JLabel("<HTML><span style = 'font:bold 15px;'>◆자가격리대상자의 가족 및 동거인을 위한 생활수칙◆</span><BR/><BR/>"
				+ "  ▶ 가족 또는 동거인은 최대한 자가격리대상자와 접촉하지 않도록 하기<BR/>"
				+ "      - 특히, 노인, 임산부, 소아, 만성질환, 암 등 면역력이 저하된 분은 접촉 금지하기<BR/>"
				+ "      - 외부인의 방문 제한하기<BR/><BR/>"
				+ "  ▶ 자가격리대상자와 불가피하게 접촉할 경우 마스크를 쓰고 <BR/>"
			    + "      서로 2m이상의 거리를 두기<BR/><BR/>"
				+ "  ▶ 자가격리대상자와 독립된 공간에서 생활하시고,<BR/>"
				+ "      공용으로 사용하는 공간은 자주 환기를 시키기<BR/><BR/>"
				+ "  ▶ 비누 또는 손세정제를 이용하여 흐르는 물에 30초 이상 자주 손 씻기<BR/><BR/>"
				+ "  ▶ 자가격리대상자와 생활용품을 구분하여 사용하기<BR/>"
				+ "      - 자가격리대상자의 의복 및 침구류는 단독세탁<BR/>"
				+ "      - 자가격리대상자의 식기류 등은 별도로 분리하여<BR/>"
				+ "       깨끗이 씻기 전에 다른 사람이 사용하지 않도록 하기<BR/><BR/>"
				+ "  ▶ 테이블 위, 문손잡이, 욕실기구, 키보드, 침대 옆 테이블 등<BR/>"
				+ "      손길이 많이 닿는 곳의 표면 자주 닦기<BR/><BR/>"
